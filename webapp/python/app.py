    ...
    try:
        ...
        with conn.cursor() as c:
            if item_id > 0 and created_at > 0:
                # paging
                sql = "SELECT * FROM `items`  LEFT OUTER JOIN `users` ON `items`.`seller_id` = `users`.`id` LEFT OUTER JOIN `categories` ON `items`.`category_id` = `categories`.`id` WHERE `status` IN (%s, %s) AND (`items.created_at` < %s OR (`items.created_at` <= %s AND `items.id` < %s)) ORDER BY `items`.`created_at` DESC, `items`.`id` DESC LIMIT %s"
                c.execute(sql, (
                ...
                # 1st page
                sql = "SELECT * FROM `items` LEFT OUTER JOIN `users` ON `items`.`seller_id` = `users`.`id` LEFT OUTER JOIN `categories` ON `items`.`category_id` = `categories`.`id` WHERE `status` IN (%s, %s) ORDER BY `items`.`created_at` DESC, `items`.`id` DESC LIMIT %s"
                c.execute(sql, (
                    Constants.ITEM_STATUS_ON_SALE,
                    Constants.ITEM_STATUS_SOLD_OUT,
                    Constants.ITEMS_PER_PAGE + 1
